...
(async () => {
  ...
  // ─── 图1: 封面 马可波罗卡片 (暗色模式) ───
  await shot('marcopolo-01-card-cover.png', `${LIVE}/cards.html`, { wait: 5000 });
  ...
  try {
    ...
    await p2.screenshot({ path: path.join(OUT, 'marcopolo-02-card-light.png'), fullPage: true, timeout: 45000 });
    console.log('    ✅ marcopolo-02-card-light.png');
  } catch (e) {
  ...
  // ─── 图3: story5 马可波罗故事页 (blockTiles) ───
  await shot('marcopolo-03-story-chapter.png', `${LIVE}/story5.html`, { wait: 5000, blockTiles: true });

  // ─── 图4: story5 noMap 纯叙事卡 ───
  await shot('marcopolo-04-story-narrative.png', `${LIVE}/story5.html`, { wait: 4000, noMap: true, blockTiles: true });

  // ─── 图5: 主页 index (带地图) ───
  await shot('marcopolo-05-index.png', LIVE, { wait: 6000, blockTiles: true });

  // ─── 图6: 主页 index noMap ───
  await shot('marcopolo-06-index-content.png', LIVE, { wait: 4000, noMap: true, blockTiles: true });
