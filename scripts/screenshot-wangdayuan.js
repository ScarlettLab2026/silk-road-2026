...
(async () => {
  ...
  // ─── 图1: 封面 汪大渊卡片 (暗色模式) ───
  await shot('wangdayuan-01-card-cover.png', `${LIVE}/cards.html`, { wait: 5000 });
  ...
  try {
    ...
    await p2.screenshot({ path: path.join(OUT, 'wangdayuan-02-card-light.png'), fullPage: true, timeout: 45000 });
    console.log('    ✅ wangdayuan-02-card-light.png');
  } catch (e) {
  ...
  // ─── 图3: story7 汪大渊 (blockTiles) ───
  await shot('wangdayuan-03-story-chapter.png', `${LIVE}/story7.html`, { wait: 5000, blockTiles: true });

  // ─── 图4: story7 noMap 纯叙事卡 ───
  await shot('wangdayuan-04-story-narrative.png', `${LIVE}/story7.html`, { wait: 4000, noMap: true, blockTiles: true });

  // ─── 图5: 主页 index (带地图) ───
  await shot('wangdayuan-05-index.png', LIVE, { wait: 6000, blockTiles: true });

  // ─── 图6: 主页 index noMap ───
  await shot('wangdayuan-06-index-content.png', LIVE, { wait: 4000, noMap: true, blockTiles: true });
